...
export const withUser = async (req: Request & SocialProviderHeader, res: Response, next: NextFunction): Promise<void> => {
  const firebaseId = req.provider.providerId;
  const user = await Repository.with(User).findOne({ providerId: firebaseId } as object);
...
export const withFirebaseUser = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  ...
  if (!firebaseId || !firebaseToken || !firebaseProvider) {
    res.status(400).json({ error: 'id and token headers are required headers' });
    return;
  }

  if (Array.isArray(firebaseId) || Array.isArray(firebaseToken) || Array.isArray(firebaseProvider)) {
    res.status(400).json({ error: 'id and token headers are not arrays' });
    return;
  }

  const decodedIdToken: DecodedIdToken = await auth().verifyIdToken(firebaseToken, false);
